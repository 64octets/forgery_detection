...
function output = CrossFeatureTest_rgb_chroma (img_path, resaved_img_path, block_size, q_multi_factor)

    FEATURE = 2; % 1 for quantization, 2 for noise estimation, 3 for interpolation, 4 for kurtosis
    COMBINATION = 1; % 1-G:B; 2-R:B; 3-R:G
    ...
    for i = 1:w
        ...
        if COMBINATION == 1
            vect = [G_local(i,j) Cr_local(i,j) 0];
        elseif COMBINATION == 2
            vect = [G_local(i,j) Cb_local(i,j) 0];
        elseif COMBINATION == 3
            vect = [Cr_local(i,j) Cb_local(i,j) 0];
        end
